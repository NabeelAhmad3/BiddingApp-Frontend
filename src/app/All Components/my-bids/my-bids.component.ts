...
export class MyBidsComponent implements OnInit {
  ...
  loadMyBids(): void {
    this.http.get<any[]>(`http://localhost:5000/product_bid/myBids/${this.userid}`).subscribe({
      next: (data) => {
  ...
  loadMyWonBids(): void {
    this.http.get<any[]>(`http://localhost:5000/product_bid/myWonBids/${this.userid}`).subscribe({
      next: (data) => {
